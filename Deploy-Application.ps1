...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		## Show Welcome Message, close VLC, verify there is enough disk space to complete the install, and persist the prompt
		...
		$VLCPaths = @("${envProgramFilesX86}\VideoLAN\VLC","${envProgramFiles}\VideoLAN\VLC")
		Foreach ($path in $VLCPaths) {
			If (Test-Path -LiteralPath (Join-Path -Path $path -ChildPath "vlc.exe") -PathType 'Leaf') {
				Write-Log -Message 'VLC was detected and will be uninstalled.' -Source $deployAppScriptFriendlyName
				Execute-Process -Path "$path\uninstall.exe" -Parameters '/S' -WindowStyle 'Hidden'
			}
		}
		...
		$exitCode = Execute-Process -Path "vlc-$appVersion-win32.exe" -Parameters '/S /L=1033 --no-qt-privacy-ask --no-qt-updates-notif' -WindowStyle 'Hidden'
